...
function select(item: ISupportedGames) {
    ...
    ipcRenderer.invoke("select-file", arg).then((arg: string[]) => {
        if (arg.length > 0) {
            let folder = settings.settings.selectGameByFolder
                ? arg[0]
                : dirname(arg[0]);
            ...
            if (typeof item.gameExe == "string") {
                ...
                // 判断 item.gameExe 是否存在于 files 中
                let exe = item.gameExe.find((item) =>
                    files.includes(item.name)
                );
                if (exe) {
                    // console.log(exe);
                    settings.settings.managerGame = item;
                    settings.settings.managerGame.gamePath = join(
                        folder,
                        exe.rootPath
                    );
                } else {
