...
watch(modType, () => {
    if (modType.value != "Custom") {
        form.value.modType = modType.value as
            | "UnityGame.modType"
...
watch(checkModType, () => {
    if (checkModType.value != "Custom") {
        form.value.checkModType = checkModType.value as
...
function addCheckModType() {
    let type = ref<ICheckModType>(
        JSON.parse(JSON.stringify(InitialCheckModTypeValue))
    );
    ElMessageBox({
        title: "添加检查类型",
        draggable: true,
...
async function submitCover(base64: string) {
    const file = await fetch(base64)
        .then((res) => res.blob())
        .then((blob) => new File([blob], "Cropped.png", { type: "image/png" }));
    let fd = new FormData();
    fd.append("file", file);

    fetch("https://assets-mod.3dmgame.com/upload/fileUpload", {
        method: "POST",
        body: fd,
    })
        .then((res) => res.json())
        .then((data) => {
            form.value.gameCoverImg = `https://assets-mod.3dmgame.com/static/upload/mod/${data.url}@webp`;
            ...
            <v-card-text>
                <el-form label-width="100">
                    <el-form-item :label="$t('Game ID')" required>
                        <el-input
                            type="number"
                            v-model="form.GlossGameId"
                            :placeholder="$t('Game ID Tip')"
                        >
                        </el-input>
                    </el-form-item>
                    ...
                    >
                        <el-input
                            ...
                            :placeholder="$t('Installation Directory Tip')"
                        ></el-input>
                    </el-form-item>
                    ...
                    <el-form-item :label="$t('Exe Name')" required>
                        <el-row>
                            <el-col :span="24">
                                <el-switch
                                    ...
                                    style="
                                        ...
                                        --el-switch-off-color: #1565c0;
                                    "
                                    :active-text="$t('Advanced')"
                                    :inactive-text="$t('Easy')"
                                />
                            </el-col>
                            <el-col :span="24" v-if="moreGameExe">
                                ...
                                <div
                                    class="star-exe-list"
                                    v-for="item in (form.gameExe as IGameExe[])"
                                    :key="item.name"
                                >
                                    {{ item.name }}
                                    <v-icon
                                        class="del"
                                        @click="delGameExe(item)"
                            ...
                            <el-col :span="24" v-else>
                                <el-input
                                    v-model="(form.gameExe as string)"
                                    placeholder="游戏主程序名称"
                                ></el-input>
                            </el-col>
                        </el-row>
                    ...
                    <el-form-item :label="$t('Startup method')" required>
                        <el-row>
                            ...
                            >
                                    ...
                                    <v-card color="#0000">
                                        <el-form label-width="80">
                                            <el-form-item label="名称">
                                                <el-input
                                                    v-model="startExe.name"
                                                    placeholder="显示的名称"
                                                ></el-input>
                                            </el-form-item>
                                            <el-form-item label="路径">
                                                <el-input
                                                    v-model="startExe.exePath"
                                                    placeholder="根目录的相对路径"
                                                ></el-input>
                                            </el-form-item>
                                            <el-form-item label="启动选项">
                                                <el-input
                                                    v-model="startExe.options"
                                                    placeholder="启动项"
                                                ></el-input>
                                            </el-form-item>
                                            <el-form-item label="cmd">
                                                <el-input
                                                    v-model="startExe.cmd"
                                                    placeholder="启动指令"
                                                ></el-input>
                                            </el-form-item>
                                        </el-form>
                                        <template #actions>
                                            <el-button @click="addStartExe"
                                                >添加</el-button
                                            >
                                        </template>
                                    </v-card>
                                ...
                                <div
                                    class="star-exe-list"
                                    v-for="item in (form.startExe as IStartExe[])"
                                    :key="item.name"
                                >
                                    {{ item.name }}
                                    <v-icon
                                        class="del"
                                        @click="delStartExe(item)"
                                        >mdi-trash-can-outline</v-icon
                                    >
                                </div>
                            </el-col>
                            <el-col :span="24" v-else>
                                <el-input
                                    v-model="(form.startExe as string)"
                                    placeholder="游戏 exe 名称"
                                ></el-input>
                            </el-col>
                        </el-row>
                    </el-form-item>
                        ...
                        <el-row>
                            <el-col :span="24">
                                <div class="cover" @click="uploadCover = true">
                                    <el-image
                                        v-if="form.gameCoverImg"
                                        :src="form.gameCoverImg"
                                    ></el-image>
                                    <el-icon v-else>
                    ...
                    <el-form-item :label="$t('Type')" required>
                        <el-select v-model="modType">
                            <el-option
                                label="通用 unity"
                                value="UnityGame.modType"
                            ></el-option>
                            <el-option
                                label="通用 unity ILCPP2"
                                value="UnityGameILCPP2.modType"
                            ></el-option>
                            <el-option
                                label="通用 虚幻"
                                value="UnrealEngine.modType"
                            ></el-option>
                            <el-option
                                :label="$t('Customize')"
                                value="Custom"
                        ...
                        <v-row>
                            <v-col
                                cols="12"
                                v-if="modType == 'UnrealEngine.modType'"
                            >
                                <el-form label-width="100">
                                    <el-form-item label="独特目录">
                                        <el-input
                                            v-model="
                                                form.unrealEngineData.bassPath
                                            "
                                            placeholder="根目录中与其他游戏不同的文件夹"
                                        ></el-input>
                                    </el-form-item>
                                    <el-form-item label="使用 UE4SS">
                                        <el-switch
                                            v-model="
                                                form.unrealEngineData.useUE4SS
                                            "
                                        ></el-switch>
                                    </el-form-item>
                                </el-form>
                            ...
                            >
                                ...
                                <v-chip
                                    variant="text"
                                    label
                                    v-for="item in (form.modType as IType[])"
                                    :key="item.id"
                    ...
                    <el-form-item :label="$t('Check type')" required>
                        <el-select v-model="checkModType">
                            <el-option
                                label="通用 unity"
                                value="UnityGame.checkModType"
                            ></el-option>
                            <el-option
                                label="通用 unity ILCPP2"
                                value="UnityGameILCPP2.checkModType"
                            ></el-option>
                            <el-option
                                label="通用 虚幻"
                                value="UnrealEngine.checkModType"
                            ></el-option>
                            <el-option
                                :label="$t('Customize')"
                                value="Custom"
                        ...
                        <v-row>
                            ...
                            >
                                ...
                                <v-chip
                                    variant="text"
                                    label
                                    v-for="item in (form.checkModType as ICheckModType[])"
                                    :key="item.TypeId"
                                    @click="editCheckModType(item)"
                                >
                                    {{ item.TypeId }}
                                    <el-button
                                        link
                                        @click.stop="delCheckModType(item)"
                                        ><v-icon
                                            >mdi-trash-can-outline</v-icon
                                        ></el-button
                                    >
