    ...
    for (const rule of game.checkModType) {
      const matched = mod.modFiles.some((file) => {
        ...
        return rule.Keyword.some((keyword) => {
          ...
          return [normalizedKeyword, `.${normalizedKeyword}`].includes(
            `.${getFileExtension(normalizedFile)}`,
          );
        });
      });
...
async function materializeImportSource(
  ...
  return FileHandler.copyFile(source.path, await join(targetFolder, fileName));
}
...
export async function importLocalModSources(
  ...
  for (const source of sources) {
    const shouldOverwrite =
      source.duplicateStrategy === "overwrite" && Boolean(source.targetMod);
    ...
    const stagingFolder = shouldOverwrite
      ? await join(manager.managerRoot, `.gmm-import-${modId}-${Date.now()}`)
      : targetFolder;
    ...
    try {
      ...
      const imported = await materializeImportSource(source, stagingFolder);
      ...
      manager.managerModList = shouldOverwrite
        ? manager.managerModList.map((item) => (item.id === modId ? mod : item))
