...
export const useManager = defineStore("Manager", {
    ...
    getters: {
        ...
        filterModList(state) {
            ...
            let list = state.managerModList.filter(
                (item) =>
                    item?.modName
                        .toLowerCase()
    ...
    actions: {
        ...
        async addModFile(file: string) {
            ...
            if (allowedExtensions.some((ext) => file.endsWith(ext))) {
                ...
                if (this.isAdded(md5)) {
                    ElMessage.error(
                        `您已经添加过『${path.basename(file)}』这款Mod了!`
                    );
                    return;
                }
                ...
                if (this.isAdded(md5)) {
                    ElMessage.error(
                        `您已经添加过『${path.basename(Folder)}』这款Mod了!`
                    );
                    return;
                }
                let target = path.join(
        ...
        async addModByTask(task: IDownloadTask, modStorage?: string) {
            ...
            if (this.isAdded(md5)) {
                ElMessage.error(
                    `您已经添加过 『${path.basename(task.name)}』 这款Mod了!`
                );
                return;
            }
            ...
            if (task.modType) {
                ...
                if (
                    typeof settings.settings.managerGame?.checkModType ==
                    "function"
                ) {
                    const extype = ExpandsType.checkModType(
                        settings.settings.managerGame.gameName,
                        files
                    );
                    if (extype) {
                        ...
                        mod.modType =
                            await settings.settings.managerGame?.checkModType(
                                mod
                            );
                    }
                }
        ...
        async addModInfo(
            ...
            if (
                typeof settings.settings.managerGame?.checkModType == "function"
            ) {
                const extype = ExpandsType.checkModType(
                    settings.settings.managerGame.gameName,
                    files
                ...
                if (extype) {
                    ...
                    mod.modType =
                        await settings.settings.managerGame?.checkModType(mod);
                }
            }
        ...
        async getModInfo() {
            ...
            this.managerModList = (await Manager.getModInfo(
                savePath
            )) as IModInfo[];
        },
        // 删除Mod
        deleteMod(mod: IModInfo) {
            ...
            let modIndex = this.managerModList.findIndex(
                (item) => item.id == mod.id
            );
            // console.log(savePath);
        ...
        async getTagsList() {
            ...
            this.tags = (await Manager.getModInfo(
                savePath,
                "tags.json"
            )) as ITag[];
        },
        ...
        async addModByGmm(file: string) {
            ...
            const settings = useSettings();
            let info: any = JSON.parse(
                await Unzipper.readZipFile(file, "info.json")
            );
            this.installLoading = false;

            if (info.gameID != settings.settings.managerGame?.GlossGameId) {
                ElMessage.error(
                    `该 .GMM 包并不属于${settings.settings.managerGame?.gameName}, 请先选择正确的游戏.`
                );
                return;
            }
            console.log(info);
        ...
        async checkAllModUpdate() {
            ...
            this.managerModList.forEach((item) => {
                if (item.webId && item.from == "GlossMod")
                    modId.push(item.webId as number);
                if (item.from == "NexusMods") nexusModsList.push(item);
            });

            if (modId.length > 0) {
                const { data } = await _3DMApi.checkModUpdate(modId);
                data.data.forEach(
                    (item: { id: number; mods_version: string }) => {
                        const mod = this.getModInfoByWebId(item.id);
                        if (mod) {
            ...
            if (nexusModsList.length > 0) {
                ...
                nexusModsList.forEach(async (item) => {
                    if (!item.isUpdate && item.other) {
                        const data = await nexusMods.getModData(
                            item.other.modId,
                            item.other.domainName
                        );
