...
export const useManager = defineStore("Manager", () => {
  ...
  function sortModsByWeight(list: IModInfo[]) {
    return list
      ...
      .sort((left, right) => {
        const weightDiff =
          Number(left.mod.weight ?? 0) - Number(right.mod.weight ?? 0);
  ...
  const filteredMods = computed<IModInfo[]>(() => {
    ...
    return sortModsByWeight(managerModList.value)
      ...
      .filter((mod) => {
        ...
        return (mod.tags ?? []).some((tag) => tag.name === selectedTag.value);
      })
      .filter((mod) => {
        if (!keyword) {
          return true;
    ...
    .then(async (games) => {
      ...
      if (managerGame.value) {
        const matchedGame = games.find(
          (game) => game.GlossGameId === managerGame.value?.GlossGameId,
        );
        if (matchedGame) {
          managerGame.value = matchedGame;
        }
      }
      console.log(managerGame.value);
    })
    .catch((error: unknown) => {
