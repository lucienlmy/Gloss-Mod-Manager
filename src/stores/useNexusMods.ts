...
export const useNexusMods = defineStore('NexusMods', {
    state: () => ({
        ...
        sort: { label: 'Update time', value: { updatedAt: { direction: "DESC" } } }
    }),
    getters: {
    ...
    actions: {
        ...
        async getModList() {
            ...
            const variables: any = {
                ...
                filter: {
                    gameDomainName: {
                        op: "EQUALS",
                        value: settings.settings.managerGame?.nexusMods?.game_domain_name
                    },
                },
        ...
        async getFileList(mod: INexusMods) {
            ...
            if (primary_files.length == 0) {
                ...
                primary_files.sort((a, b) => b.uploaded_timestamp - a.uploaded_timestamp)
            }
