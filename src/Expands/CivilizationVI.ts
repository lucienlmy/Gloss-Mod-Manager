...
export const supportedGames = async () => ({
    GlossGameId: 5,
    steamAppID: 289070,
    installdir: await join("Sid Meier's Civilization VI", "Base", "Binaries", "Win64Steam"),
    ...
    startExe: [
        ...
        {
            name: "直接启动",
            exePath: await join("Base", "Binaries", "Win64Steam", "CivilizationVI.exe")
        }
    ],
    archivePath: await join(await FileHandler.getMyDocuments(), "My Games", "Sid Meier's Civilization VI"),
    gameCoverImg: "https://assets-mod.3dmgame.com/static/upload/game/5.jpg",
    modType: [
        {
            id: 1,
            name: "mods",
            installPath: await join(await FileHandler.getMyDocuments(), "My Games", "Sid Meier's Civilization VI", "Mods"),
            async install(mod) {
                void mod;
