...
async function writeDlcName(name: string, isInstall: boolean) {
    ...
    const xml = await await RpfHeader("Read", {
        rpf: update_path,
        filename: dlclistXml,
    });
    ...
    if (data) {
        ...
        const res = await RpfHeader("Write", {
            rpf: update_path,
            inputFile: inputFile,
...
<CPedModelInfo__InitDataList>
    ...
    if (
        !Array.isArray(PedModelInfo.CPedModelInfo__InitDataList.InitDatas.Item)
    ) {
        PedModelInfo.CPedModelInfo__InitDataList.InitDatas.Item = [
            PedModelInfo.CPedModelInfo__InitDataList.InitDatas.Item,
        ];
