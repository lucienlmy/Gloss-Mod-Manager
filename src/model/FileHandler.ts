...
export class FileHandler {
    ...
    public static getMyDocuments() {
        try {
            ...
            const output = execSync(regQueryCommand, {
                encoding: "utf8",
            }).toString();
     ...
     * @returns
     */
    public static getAllFilesInFolder(
        folderPath: string,
     ...
     * @returns
     */
    public static getAllFolderInFolder(
        folderPath: string,
        subdirectory: boolean = false
    ) {
        let res: string[] = [];
        ...
        files.forEach((file) => {
            ...
            if (stat.isDirectory()) {
                res.push(filePath);
                if (subdirectory) {
                    const subFolders = this.getAllFolderInFolder(
                        filePath,
                        true
                    );
    ...
    public static compareFileName(name1: string, name2: string) {
        return (
            path.basename(name1).toLowerCase() ==
            path.basename(name2).toLowerCase()
        );
    }
