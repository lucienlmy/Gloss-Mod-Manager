...
let list = computed<ISupportedGames[]>(() => {
    const list = manager.supportedGames.map((item) => ({
        name: item.gameShowName || t(item.gameName),
        game: item,
    }));
...
function select(all: boolean) {
    if (all) {
        settings.settings.tourGameList = list.value.map(
            (item) => item.GlossGameId
        );
    } else {
        // 反选
        settings.settings.tourGameList = list.value
            .filter(
                (item) =>
                    !settings.settings.tourGameList.includes(item.GlossGameId)
            )
            .map((item) => item.GlossGameId);
    }
}
                ...
                <v-col cols="12" class="top">
                    <div class="title">
                        <div class="text">
                            ...
                            <small
                                >({{
                                    ` ${settings.settings.tourGameList.length}/${list.length}`
                                }})</small
                            >
                            <v-chip
                                label
                    ...
                    <div class="close">
                        <v-chip
                            label
                            append-icon="mdi-close"
                            @click="explore.showTourGameListDialog = false"
                            variant="text"
                            >{{ $t("Close") }}</v-chip
                        >
                    </div>
                ...
                <v-card v-if="settings.settings.tourGameList.length > 0">
                    ...
                    <v-card-text>
                        <v-row class="selected-games">
                            <v-col
                                cols="2"
                                v-for="item in settings.settings.tourGameList"
                                :key="item"
                            >
                                <GlossModSelectedGames
                                    :id="item"
                                ></GlossModSelectedGames>
                            </v-col>
                        </v-row>
                            ...
                            >
                                <v-row no-gutters>
                                    ...
                                    <v-col class="game-name" cols="12">{{
                                        item.gameShowName || $t(item.gameName)
                                    }}</v-col>
                                </v-row>
                            </v-col>
                        </v-item>
