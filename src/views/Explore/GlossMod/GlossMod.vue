...
<template>
    <Search>
        <v-text-field
            v-model="explore.searchText"
            class="search-input"
            :placeholder="$t('You can search for what you want here')"
            variant="underlined"
            @keydown.enter="explore.search"
        ...
        <v-card-text>
            <v-row
                v-if="
                    ...
                    settings.settings.tourGameList.length == 0
                "
                class="empty"
            >
                <div
                    @click="explore.showTourGameListDialog = true"
                    class="empty-hint"
                >
                    {{ $t("Please select a game first") }}
                    <v-icon>mdi-plus</v-icon>
                </div>
            </v-row>
            <v-row
                class="mod-wrap"
                v-else-if="explore.mods.length > 0 && !explore.loading"
            >
                <v-col
                    cols="12"
                    sm="6"
