...
let mod_types = computed(() => {
    let list = [{ text: t("All"), value: 0 }, ...explore.gameTypeList];
    return list;
});
</script>
<template>
    <v-row class="filter">
        <v-col cols="12" sm="4" md="2">
            <v-select
                hide-details="auto"
                :label="t('Type of creation')"
                v-model="explore.original"
                :items="original_filter"
                item-title="text"
                item-value="value"
                variant="solo"
            ></v-select>
