...
// 全选
function selectAll() {
    manager.shareList = manager.managerModList.filter(
        (item) => item.webId || item.other?.namespace
    );
}
...
function reverseSelection() {
    manager.shareList = manager.managerModList.filter(
        (item) =>
            (item.webId || item.other?.namespace) &&
            !manager.shareList.includes(item)
    );
}
        ...
        message: () =>
            h(ElCol, { cols: 24 }, [
                ...
                h(
                    "div",
                    { style: "opacity: .5" },
                    "复制代码后打开 GMM 使用 Ctrl+V 粘贴代码即可导入内容"
                ),
            ]),
    ...
    <Dialog v-model="manager.showShare">
        ...
        <v-col cols="12">
            <v-btn variant="text" @click="selectAll"
                >全选 <v-icon>mdi-check</v-icon>
            </v-btn>
            <v-btn variant="text" @click="reverseSelection"
                >反选 <v-icon>mdi-close</v-icon></v-btn
            >
        </v-col>
        <div class="list">
            <el-checkbox-group v-model="(manager.shareList as any[])">
                <v-list-item
                    v-for="item in manager.managerModList"
                    :key="item.id"
                >
                    <el-checkbox
                        :value="item"
                        :disabled="!(item.webId || item.other?.namespace)"
                    >
                        <v-list-item-title
                            :title="
                                !(item.webId || item.other?.namespace)
