...
<template>
    <v-chip
        ...
        @click="download.showAddTaskDialog = !download.showAddTaskDialog"
        label
        variant="text"
    >
        {{ $t("New Download") }}</v-chip
    >
    <el-dialog
        v-model="download.showAddTaskDialog"
        draggable
    ...
    >
        ...
        <el-form label-width="120" v-model="form">
            <el-form-item :label="$t('Website')">
                <el-input
                    v-model="form.modWebsite"
                    :placeholder="$t('Website Tip')"
                >
                    <template #append>
                        <el-button @click="parsing" :loading="loading">{{
                            $t("Analyze")
                        }}</el-button>
                    </template>
                </el-input>
            ...
            <el-form-item :label="$t('Tag')">
                <div class="tags">
                    ...
                    >
                        ...
                        <template #reference>
                            <el-button @click="visible = true" link
            ...
            <el-form-item
                :label="$t('Type')"
                v-if="settings.settings.managerGame"
            >
                <el-select v-model="form.modType">
                    <el-option
                        v-for="item in settings.settings.managerGame?.modType"
                        :key="item.id"
                        :label="item.name"
                        :value="item.id"
                    />
        ...
        <v-card-actions>
            ...
            <v-btn @click="download.showAddTaskDialog = false">{{
                $t("Cancel")
            }}</v-btn>
        </v-card-actions>
        <!-- </el-card> -->
