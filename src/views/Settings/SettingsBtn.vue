...
async function exportLang() {
    ...
    languages.forEach((lang) => {
        ...
        let data = {
            data: {
                name: settings.langList.find((item) => item.value == lang)
                    ?.text,
                code: lang,
                author: "",
    ...
    <v-row>
        ...
        <v-col cols="12">
            ...
            <v-chip
                label
                variant="text"
                append-icon="mdi-folder-download-outline"
                @click="openDownloadCache"
            >
            ...
            <v-chip
                ...
                append-icon="mdi-folder-arrow-right-outline"
        ...
        <v-col cols="12">
            <v-chip
                v-if="!user.nexusModsUser.key"
                label
                variant="text"
                @click="user.loginNexusModsUser"
            >
                {{ $t("Login") }} NexusMods</v-chip
            >
            <template v-else>
                <v-chip label variant="text"
                    >{{ $t("Logined") }}: {{ user.nexusModsUser.name }}</v-chip
                >
                <v-chip
                    label
                    variant="text"
                    @click="user.logoutNexusModsUser"
                    >{{ $t("Logout") }}</v-chip
                >
            </template>
        </v-col>
